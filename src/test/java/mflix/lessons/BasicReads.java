...
public class BasicReads extends AbstractLesson {
    ...
    public void testCanFindOne() {
        ...
        assertNotEquals("should not be null", unexpected, actual);

        // we expect the iterator to have nothing left.
        assertFalse("the iterator should have no next", cursor.hasNext());
    }
    ...
    public void testFindOneSalmaHayek() {
        ...
        Document actualFilters = moviesCollection.find(eq("cast", "Salma Hayek")).limit(1).iterator().next();
        ...
        assertEquals(expectedTitle, actualFilters.getString("title"));
        assertEquals(expectedYear, (int) actualFilters.getInteger("year"));
    ...
    public void testProjection() {
        ...
        Document resultUsingFiltersAndProjections =
                moviesCollection
                .find(eq("cast", "Salma Hayek"))
                .limit(1)
        ...
        assertEquals(3, resultUsingFiltersAndProjections.keySet().size());

        // And let's make sure they are what we expected
        assertTrue(result.keySet().containsAll(Arrays.asList("_id", "title", "year")));
        assertTrue(resultUsingFiltersAndProjections.keySet().containsAll(Arrays.asList("_id", "title", "year")));
    }
    ...
    public void testProjectsAway_id() {
        ...
        List<Document> resultsUsingFiltersAndProjections = new ArrayList<>();
        moviesCollection
                .find(all("cast", "Salma Hayek", "Johnny Depp"))
                .projection(and(include("title", "year"), excludeId()))
                .into(resultsUsingFiltersAndProjections);

        // There should only be 1 result in our dataset
        assertEquals(1, results.size());
        assertEquals(1, resultsUsingFiltersAndProjections.size());

        // Now we should only have 2 keys, title and year
        Document firstResult = resultsUsingFiltersAndProjections.get(0);
        assertEquals(2, firstResult.keySet().size());
        assertTrue(firstResult.keySet().containsAll(Arrays.asList("title", "year")));

        Document firstResultUsingFiltersAndProjections = resultsUsingFiltersAndProjections.get(0);
        assertEquals(2, firstResultUsingFiltersAndProjections.keySet().size());
        assertTrue(firstResultUsingFiltersAndProjections.keySet().containsAll(Arrays.asList("title", "year")));
